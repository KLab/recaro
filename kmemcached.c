...
static ushort mc_port = DEFAULT_PORT;
module_param(mc_port, ushort, S_IRUGO);
...
static void listen_work(struct work_struct *work){
    ...
    while (1){
        ...
        kernel_setsockopt(new_sock, SOL_TCP, TCP_NODELAY, (void*)&yes, sizeof(yes));
...
static int open_listen_socket(void){
    ...
    listen_address.sin_port      = htons(mc_port);
